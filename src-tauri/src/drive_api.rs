...
pub async fn get_or_create_backup_folder(access_token: &str) -> Result<String, String> {
    ...
    let url = format!("https://www.googleapis.com/drive/v3/files?q={}&spaces=drive", urlencoding::encode(query));
    ...
    let list: DriveFileList = res.json().await.map_err(|e| format!("Failed to parse JSON: {}", e))?;
    ...
    let created: DriveFile = res.json().await.map_err(|e| format!("Failed to parse created folder: {}", e))?;
    Ok(created.id)
}

pub async fn upload_database(access_token: &str, folder_id: &str, db_path: PathBuf) -> Result<String, String> {
    let client = Client::new();

    let mut file = File::open(&db_path).await.map_err(|e| format!("Failed to open DB file: {}", e))?;
    let mut file_contents = Vec::new();
    file.read_to_end(&mut file_contents).await.map_err(|e| format!("Failed to read DB file: {}", e))?;
    ...
    let file_part = multipart::Part::bytes(file_contents)
        ...
        .mime_str("application/x-sqlite3").unwrap();
    ...
    let uploaded: DriveFile = res.json().await.map_err(|e| format!("Failed to parse upload response: {}", e))?;
    Ok(uploaded.id)
}
