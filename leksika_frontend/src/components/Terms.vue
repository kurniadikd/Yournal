...
onMounted(async () => {
  try {
    ...
    const response = await fetch(
      `${serverBaseUrl}/media/app/cookies/Terms_of_Service.html`,
    );
    ...
    tosContent.value = bodyMatch ? bodyMatch[1] : html;
  } catch (e) {
...
<template>
  <div class="min-h-screen flex flex-col bg-[var(--color-background)]">
    ...
    <main class="flex-grow container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div 
        v-else 
        class="tos-content bg-[var(--color-surface-container)] rounded-2xl p-6 md:p-8 shadow-lg"
        v-html="tosContent"
      ></div>
    </main>
...
.tos-content :deep(a:hover) {
  color: var(--color-primary-fixed-dim);
}

.tos-content :deep(.highlight) {
...
.tos-content :deep(.highlight p) {
  color: var(--color-on-primary-container);
  margin-bottom: 0.5rem;
}

.tos-content :deep(.toc) {
...
.tos-content :deep(.toc ol) {
  columns: 2;
}

.tos-content :deep(.toc li) {
  margin-bottom: 0.3rem;
  font-size: 0.9rem;
}

.tos-content :deep(.last-updated) {
  color: var(--color-on-surface-variant);
  font-size: 0.9rem;
  margin-bottom: 2rem;
}
...
.tos-content :deep(body) {
  all: unset;
}

.tos-content :deep(style) {
  display: none;
}
